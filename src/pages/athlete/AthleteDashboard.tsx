...
import { Target, Dumbbell, TrendingUp, CheckCircle2, Clock, Loader2, Video, Mail, UserPlus, XCircle, Plus, BookOpen, Users } from "lucide-react";
...
const AthleteDashboard = () => {
  ...
  const inviteParentMutation = useMutation({
    mutationFn: async () => {
      if (!parentInviteName.trim() || !parentInviteEmail.trim()) throw new Error("Name and email are required");

      // Look up coach linked to this athlete
      ...
      // Insert invite and get the token back
      const { data: invite, error } = await (supabase as any)
        .from("parent_invites")
        .insert({
          coach_id: coachId,
          athlete_user_id: user!.id,
          parent_name: parentInviteName.trim(),
          parent_email: parentInviteEmail.trim().toLowerCase(),
        })
        .select("token")
        .single();
      if (error) throw error;
      ...
      await supabase.functions.invoke("send-parent-invite-email", {
        body: { parentEmail: parentInviteEmail.trim().toLowerCase(), parentName: parentInviteName.trim(), coachName, athleteName, signupUrl },
      });
    },
    onSuccess: () => {
  ...
  return (
    <DashboardLayout role="athlete">
          ...
          <Card>
            <CardHeader>
              <div className="flex items-center justify-between">
                ...
                <Button size="sm" variant="outline" onClick={() => setShowInviteParentDialog(true)}>
            ...
            <CardContent className="space-y-2">
              {linkedParents.length === 0 && pendingParentInvites.length === 0 ? (
                <p className="text-sm text-muted-foreground text-center py-4">No parents linked yet. Send an invite above.</p>
              ) : (
                <>
                  {linkedParents.map((p: any) => (
                    <div key={p.user_id} className="flex items-center gap-2 rounded-lg border px-3 py-2">
                      ...
                      <span className="ml-auto text-[10px] font-semibold text-emerald-400 bg-emerald-400/10 border border-emerald-400/20 rounded px-1.5 py-0.5">Linked</span>
                    </div>
                  ))}
                  {pendingParentInvites.map((inv: any) => (
                    <div key={inv.id} className="flex items-center gap-2 rounded-lg border px-3 py-2 opacity-70">
                      ...
                      <span className="ml-auto text-[10px] font-semibold text-amber-400 bg-amber-400/10 border border-amber-400/20 rounded px-1.5 py-0.5 shrink-0">Pending</span>
                    </div>
                  ))}
                </>
              )}
      ...
      <Dialog open={showInviteParentDialog} onOpenChange={(o) => !o && setShowInviteParentDialog(false)}>
        <DialogContent className="max-w-sm">
          ...
          <div className="space-y-3 py-1">
            <div className="space-y-1">
              ...
              <Input
                ...
                value={parentInviteName}
                onChange={(e) => setParentInviteName(e.target.value)}
                className="h-9"
              />
            </div>
            <div className="space-y-1">
          ...
          <DialogFooter>
            <Button variant="ghost" onClick={() => setShowInviteParentDialog(false)}>Cancel</Button>
            <Button
              onClick={() => inviteParentMutation.mutate()}
              disabled={!parentInviteName.trim() || !parentInviteEmail.trim() || inviteParentMutation.isPending}
            >
              {inviteParentMutation.isPending && <Loader2 className="h-3.5 w-3.5 animate-spin mr-1" />}
              Send Invite
            </Button>
          </DialogFooter>
        </DialogContent>
